...
5. Check these acceptance conditions:
   ...
   - **Containment and clearance.** Children fit inside their frame with title/padding space. Ancestor-descendant containment is intentional; check collisions between unrelated nodes/frames, including different stages, rather than reporting parent-child containment as overlap.
   - **Reference composition.** Stages follow the main data path, inputs stay local, repeated units share columns, and frames have meaningful short labels. Connected stages do not wrap into a decorative grid. See the style reference for permitted grouped inputs and semantic nesting.
   - **Readable wires.** Compare source output side to target input side, not node centers. Arrange ordinary dependencies left-to-right with a positive edge gap, order local feeders by visible input sockets, and keep cross-stage wires in clear corridors. Inspect actual wires for crossings and node-body occlusion; node ordering alone does not prove no crossings. Preserve required zone semantics rather than rewiring evaluation to satisfy an aesthetic rule.
6. Repair semantic membership/counts using the appropriate structured operations first. For a whole generated/reformatted tree, call `plan_node_layout` against the latest layout revision with exact `main_path` node names. It arranges nested Frames from the inside out, keeps the upper main-path corridor open, moves auxiliary inputs near consumers, and emits only layout operations. It never invents Frames/subgroups, pads counts, or changes computation. A blocked/cyclic/stale plan must be resolved before applying. Validate and apply its returned `patch` through the correct domain tools; a saved plan envelope is not itself a patch file. Preserve shared-tree and backup policy. For a local edit, use a targeted layout patch instead of this whole-tree planner.
7. Read back using `inspect_node_layout(refresh=true)` and `audit_node_layout(refresh=true)` after the apply and auto-shrink. Require complete membership, counts, bounds, containment and clearance; inspect warnings too. `predicted_audit` is advisory. Current wire checks cover endpoint direction/order, not rendered Bezier crossings or socket-level alignment. Finish by inspecting the actual Node Editor at stage and overall scale; get_viewport_screenshot captures the 3D viewport, not the node graph. Recheck each authored subgroup independently.

Report remaining issues or unavailable visual evidence explicitly. `valid=true`, distinct locations, or a count based on `node.height` alone cannot establish that the graph is visually formatted. Never declare completion with a known unframed node in the generated/formatted scope.
