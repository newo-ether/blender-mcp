...
```powershell
Set-ExecutionPolicy Bypass -Scope Process -Force; & ([scriptblock]::Create(([string](irm https://raw.githubusercontent.com/newo-ether/blender-mcp/v1.12.1/install.ps1)).TrimStart([char]0xFEFF))) -ReleaseTag v1.12.1
```
...
5. 安装到 `%LOCALAPPDATA%\BlenderMCP\venv-1.12.1` 这样的版本专用环境；
...
6. 关闭 **Allow AI control** 可把某个窗口保留给人工操作。每个 3D 视图内出现青色空心边框时，表示该 Blender 进程正被 AI 占用；边框不会拦截鼠标和键盘输入。
...
```powershell
py -3 -m venv .venv
.\.venv\Scripts\python.exe -m pip install .\blender_mcp-1.12.1-py3-none-any.whl
```
...
python -m pip install ./blender_mcp-1.12.1-py3-none-any.whl
```
