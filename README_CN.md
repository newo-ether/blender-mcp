...
`-Scope Process` 只对当前 PowerShell 窗口生效，不会永久修改用户或系统的执行策略。纯 ASCII 的 [bootstrap.ps1](bootstrap.ps1) 负责取得并启动稳定入口 [install.ps1](install.ps1)；该入口会从本地仓库或对应的 GitHub 源码归档加载 [`scripts/installer`](scripts/installer) 中按职责拆分的模块。如需固定版本、确保每次安装结果一致，请使用：
...
5. 安装到 `%LOCALAPPDATA%\BlenderMCP\venv-1.16.0` 这样的版本专用环境；
6. 将服务端和扩展安装到每个选中的 Blender 版本，同时保留原有的 Blender 偏好设置；
7. 为选中的客户端添加或更新名为 `blender_mcp` 的配置项；如有 Codex CLI 则优先使用 CLI，只有 Codex Desktop 时也会直接配置二者共用的 `~/.codex/config.toml`，不要求额外安装 CLI；
8. 为选中的 Codex 和 Claude Code 安装同一份可移植 Skill，并为 Claude Desktop 准备已校验的上传 ZIP；安装结束时明确提醒仍需手动上传，不会把它报告为已安装。

安装器可以反复运行，不会重复添加配置。完全一致的 Codex 配置会保持不变；缺少 `blender_mcp` 时会新增，同名但内容不同的 Codex、Claude Code 或 Claude Desktop 用户配置默认会被替换。如需保留自定义设置，请使用 `-PreserveExistingMcpEntries`。Codex Desktop-only 回退路径会用 Python 解析原 TOML 与候选 TOML，保留无关原文并先备份；如果遇到无法安全定位的特殊布局，则拒绝修改。
...
| 目标 | 安装器行为 |
| --- | --- |
| Codex / ChatGPT | 合并为一个选项，在二者共用的用户配置中添加或更新 `blender_mcp` stdio 服务，并在 `~/.agents/skills` 安装一份共用 Skill。只安装 Codex Desktop 即可，Codex CLI 是可选项；没有 CLI 时，安装器会校验并安全更新 `~/.codex/config.toml`，替换前先备份。 |
...
2. MCP 服务端启动时只发现实例，不会占用 Blender。首个需要实时 Blender 的操作可以自动选择唯一可用实例；有多个实例时必须用明确的 `instance_id` 调用 `claim_blender_instance`，不会根据前台窗口或端口顺序猜测。
3. 同一个 AI 任务从开始到结束持有一次占用，确保连续的读取和修改始终落在同一文件；`get_active_blender_instance` 可查看当前目标。
4. AI 在最终回复、提前停止或交还任务前调用 `release_blender_instance`。如果 Blender 已不可达，则由有界租约作为兜底并自动到期。
5. 实例心跳会跨 `.blend` 文件加载继续运行；注册表时间戳过期时，会先验证实时端点再判定实例不可用。
6. 关闭 **Allow AI control** 可把某个窗口保留给人工操作。每个 3D 视图内出现青色空心边框时，表示该 Blender 进程正被 AI 占用；当 AI 写入某类节点树后，对应的节点编辑器也会亮起边框，并标注最近一次写入操作。边框不会拦截鼠标和键盘输入。
...
| `create_node_group` | 创建空的本地几何、着色器或合成器节点组，并返回首个 patch revision。 | 是 |
| `ensure_scene_compositor_tree` | 检查准确 Scene；只有显式传入 `create_if_missing=true` 时才创建、验证缺失的合成树，并支持回滚。 | 显式创建时 |
| `ensure_geometry_nodes_modifier` | 检查准确的几何节点宿主/修改器；只有显式授权时才创建空 Mesh、修改器或改绑节点组。 | 显式创建或改绑时 |
...
2. 缺少独立节点组时调用 `create_node_group`；几何节点宿主先用 `ensure_geometry_nodes_modifier` 只读检查，再显式开启创建或改绑。
3. 调用 `list_node_trees`，保留完整的 `tree_ref`。
4. 大树先用 `get_node_tree_index` 定位节点。
5. 只导出相关节点和少量邻居；默认保留 `view="auto"`：完整图使用 `operations`，定向子图使用 `semantic`。
6. 把 `tree_ref` 和返回的 `revision` 写入小型 patch JSON。
7. 用客户端现有的文件编辑工具修改 patch。
8. 先调用 `validate_node_tree_patch`。
9. 只应用通过校验的 patch，再检查 `actual_diff`、`new_revision`、使用者和备份去向。

需要一次完成时，可用 `modify_verify_save` 组合副本校验、节点/连线/接口数量断言、事务提交和 revision 回读。默认 `save_policy="never"`；`on_success` 与 `required` 都属于显式保存请求。

通用协议支持常规节点与布局操作、Frame 注释、接口面板与 socket 元数据、Color Ramp、Curve Mapping，以及带类型的 Blender ID 和 View Layer 引用。`BLENDER_MCP_WORKSPACE` 是文件边界；越界路径、非 JSON、超过 2 MiB 或 500 个操作的 patch 都会被拒绝。完整响应上限为 8 MiB；超出时会明确引导使用索引和局部导出。
...
Codex Desktop 使用同一个 `~/.codex/config.toml`。检测到 Desktop 但没有 `codex` 命令时，安装器会写入以下等价配置：
...
- `execute_blender_code` 可以在 Blender 内执行任意 Python。在节点工作流中，它只应用于最小、已确认的结构化能力缺口，不应用来替代结构化工具可以表达的整张图。
...
| [bootstrap.ps1](bootstrap.ps1) | 纯 ASCII 的一行安装入口，用于取得本地化安装入口。 |
| [install.ps1](install.ps1) | 稳定的 Windows 安装器接口，以及本地/远程模块加载器。 |
| [scripts/installer](scripts/installer) | 按职责拆分的安装器模块：通用辅助、Release 资源、Skill、检测、目标选择、客户端注册、安全 Codex TOML 回退和主流程。 |
