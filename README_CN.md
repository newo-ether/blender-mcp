...
5. 安装到 `%LOCALAPPDATA%\BlenderMCP\venv-1.9.0` 这样的版本专用环境；
...
原有的 8 个 `*_geometry_node_*` 工具仍保留，继续支持修改器输入、显式的共享树策略以及官方 Essentials 资产查询。

### 推荐流程

1. 调用 `list_node_trees`，保留完整的 `tree_ref`。
2. 大树先用 `get_node_tree_index` 定位节点。
3. 只导出相关节点和少量邻居。
4. 把 `tree_ref` 和返回的 `revision` 写入小型 patch JSON。
5. 用客户端现有的文件编辑工具修改 patch。
6. 先调用 `validate_node_tree_patch`。
7. 只应用通过校验的 patch，再检查 `actual_diff`、`new_revision`、使用者和备份去向。
...
- 使用 Hunyuan3D 生成并导入；
- 三个官方资料查询工具；
- 6 个按归属对象工作的通用节点工具，以及 8 个 Geometry Nodes v1 工具。
...
| 完整节点树超过响应上限 | 先用 `get_node_tree_index`，再向 `export_node_tree` 传入 `node_names` 和较小的 `neighbor_depth`。 |
