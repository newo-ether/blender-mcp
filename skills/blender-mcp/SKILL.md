...
- When creating or formatting nodes, read the reference layout style linked from references/node-workflows.md before placing nodes. Use named stage frames, a left-to-right main path, and local parameter inputs.
- Encapsulate functional modules as subgroups with an explicit interface. Use Frames for processing steps inside a tree; do not turn every step into a subgroup or substitute a Frame for module encapsulation.
- Every non-Frame node in a generated or reformatted tree must have a NodeFrame parent, including Group Input, Group Output, Reroute, constants, and auxiliary nodes. Root frames may be unparented. Check each generated nested tree too.
- Each Frame must contain 4-19 direct child nodes; a nested Frame counts as one child of its parent and must satisfy the same rule itself. Group by meaningful stages, merge tiny steps, and nest complex stages. Do not add filler nodes or decorative frames to meet the count. Use the frame audit linked from references/node-workflows.md.
- Enforce the six strict formatting rules in [Strict layout constraints](references/node-layout-style.md#strict-layout-constraints): no overlap or touching between unrelated boxes; every link strictly rightward; exactly one visible used output and one outgoing link per Group Input; Group Inputs within 250 canvas units of their targets and aligned to the receiving sockets; ordered fan-in; compact vertical spacing between stacked neighbors (target 20-40 canvas units edge-to-edge, maximum 60). These are completion requirements, not preferences. Do not relax them for related parameters, local fan-out, uniform oversized rows, or a passing tool result. Never rename built-in nodes to explain their role; use Frame labels.
- Within each processing step's Frame, minimize total internal wire length among layouts satisfying every constraint. Compare legal node orders, columns, and compact placement; do not accept the first valid planner result as the shortest arrangement. Optimize nested steps from the inside out. Preserve semantic membership, computation, and socket order; shorter wires never justify violating a hard rule. See [Shortest wiring within each step](references/node-layout-style.md#shortest-wiring-within-each-step).
- Run the layout post pass after changes to nodes, links, parenting, widths, socket visibility, or other settings that affect the displayed layout. Require both complete frame membership and readable placement; patch valid=true alone does not verify layout (references/node-workflows.md, "Layout post pass").
- Prefer inspect_node_layout for measured canvas bounds and audit_node_layout for membership, 4-19 counts, containment, overlaps, and wire-order diagnostics. After semantic membership is correct, use plan_node_layout with an explicit main_path to generate a candidate patch, then validate/apply through the tree's existing transaction tools. Reinspect after redraw; predicted/cached dimensions and audit valid=true do not prove visual acceptance. These tools operate on one complete tree, so repeat for authored subgroups and do not use the whole-tree planner for an unrelated local edit.
