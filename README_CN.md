...
```powershell
Set-ExecutionPolicy Bypass -Scope Process -Force; & ([scriptblock]::Create(([string](irm https://raw.githubusercontent.com/newo-ether/blender-mcp/v1.13.0/install.ps1)).TrimStart([char]0xFEFF))) -ReleaseTag v1.13.0
```
...
5. 安装到 `%LOCALAPPDATA%\BlenderMCP\venv-1.13.0` 这样的版本专用环境；
...
```powershell
py -3 -m venv .venv
.\.venv\Scripts\python.exe -m pip install .\blender_mcp-1.13.0-py3-none-any.whl
```
...
python -m pip install ./blender_mcp-1.13.0-py3-none-any.whl
```
